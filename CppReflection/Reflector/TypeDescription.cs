...
namespace CppReflection.Reflector
{
    ...
    public class TypeDescription
    {
        ...
        public static List<TypeDescription> CreateDescription(TypeDeriverTreeNode node)
        {
        ...
        private static void CreateDescription(ref List<TypeDescription> descriptors, TypeDeriverTreeNode node, int parentId, int indent = 0)
        {
            TypeDescription desc = new TypeDescription();
            desc.Name = node.Own.Name;
            desc.FullName = node.Own.FullName.Replace("global::", "");
            desc.ParentId = parentId;
            desc.Id = descriptors.Count;
            desc.Indent = indent;
            desc.File = node.Own.VCClass.File;
            desc.IsNotAbstract = !node.Own.VCClass.IsAbstract;
